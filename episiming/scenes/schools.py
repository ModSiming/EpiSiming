...
class School:
        ...
        for j in range(len(school_age_groups_fraction)):
            for i in range(len(school_age_groups)):
                if i < len(school_age_groups_fraction[j]):
                    dist_school_age_fraction[j][school_age_groups[i]:school_age_groups[i + 1]] = \
                        school_age_groups_fraction[j][i]
    ...
    def select_students(self):
        ...
        for j in range(len(self.dist_school_age)):
            students = np.array(
                [np.random.permutation(np.arange(len(self.pop_age))[self.pop_age == i])[
                 :int(self.dist_school_age[j][i])]
